    ...
    if (Capacitor.isNativePlatform()) {
      ...
      try {
        ...
        for (const channel of [
          { id: "azkar_evening_channel_v2", name: "أذكار المساء", sound: "azkar_evening.wav" },
          { id: "azkar_sleep_channel_v2", name: "أذكار النوم", sound: "azkar_sleep.wav" },
          { id: "azkar_tahajjud_channel_v2", name: "قيام الليل والوتر", sound: "azkar_tahajjud.wav" },
        ]) {
          await LocalNotifications.createChannel({
            id: channel.id,
            name: channel.name,
      ...
      if (settings.azkarReminder && settings.eveningAzkarReminder !== false) {
        ...
        if (eveningAzkarTime.getTime() > Date.now()) {
          notificationsToSchedule.push({
            ...
            channelId: "azkar_evening_channel_v2",
      ...
      if (settings.tahajjudReminder !== false) {
        ...
        if (tahajjudTime.getTime() > Date.now()) {
          notificationsToSchedule.push({
            ...
            channelId: "azkar_tahajjud_channel_v2",
